...
void test_process_line(void) {
    ...
    // Create token and symbol tables
    TokenTable *table;
    SymbolTable *symbol_table = symbol_table_create();
    int rom_address;
    Token *token;
    ...
    table = token_table_create();
    rom_address = 0;
    assert(process_line("D=M //comment", table, symbol_table, &rom_address) == PROCESS_SUCCESS);
    token_table_reset(table);
    token = token_table_next(table);
    assert(token != NULL && token->type == KEYWORD && token->value.keyword == KW_D);
    token = token_table_next(table);
    ...
    // (LOOP) -> Label added to symbol table
    table = token_table_create();
    rom_address = 0;
    assert(process_line("(LOOP)", table, symbol_table, &rom_address) == PROCESS_SUCCESS);
    assert(symbol_table_contains(symbol_table, "LOOP"));
    assert(symbol_table_get_address(symbol_table, "LOOP") == 0);
    token_table_free(table);

    // Invalid symbol (1WRONG)
    table = token_table_create();
    rom_address = 0;
    assert(process_line("(1WRONG)", table, symbol_table, &rom_address) == PROCESS_INVALID);
    assert(!symbol_table_contains(symbol_table, "WRONG$"));
    token_table_free(table);
